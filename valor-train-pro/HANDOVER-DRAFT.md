...
| App | **https://app.valorsportsacademywa.com**, live 9/24. DNS is a CNAME `app` → `cname.vercel-dns.com` in Valor's Wix DNS. The Let's Encrypt certificate is auto-renewed by Vercel. The old `valor-train-pro.vercel.app` redirects here. Hosted on Vercel under Omar's account; test deploys still get throwaway `valor-train-…vercel.app` links. |
