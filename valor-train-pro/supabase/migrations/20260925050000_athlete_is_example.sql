-- A fully filled-in demo athlete ("Test Omar") pinned at the top of staff lists as a
-- live example of what every screen looks like. Safe to delete later.
alter table public.athletes add column if not exists is_example boolean not null default false;
update public.athletes set is_example = true where id = '99dfb1ba-e982-4777-9812-88daf5a6219a';

-- expose it on the staff view (new column at the end so the view can be replaced)
